...
public final class KafkaClient: @unchecked Sendable {
    ...
    private var clientId: String = "pubsub-viewer-client"
    
    public init(config: MQConnectionConfig) {
        self.config = config
        self.consumerGroupId = config.options["group.id"] ?? "pubsub-viewer-\(UUID().uuidString.prefix(8))"
        self.clientId = config.options["client.id"] ?? "pubsub-viewer-client"
    }
